...
def align_timestamps_for_merge(df, frequency):
    """
    Align timestamps to common boundaries for successful merging across different assets.
    This solves the issue where different assets have different timestamp patterns.
    
    Parameters:
    - df: DataFrame with 'timestamp' column
    - frequency: String frequency ('hour', '4hour', '30min', '15min', '5min', 'minute', 'day')
    ...
    # Align based on frequency
    if frequency == '4hour':
        # Round to nearest 4-hour boundary: 00:00, 04:00, 08:00, 12:00, 16:00, 20:00
        # This aligns VIX (05:00->04:00, 09:00->08:00, 13:00->12:00, 17:00->16:00)
        # with regular assets (08:00, 12:00, 16:00, 20:00)
        df['timestamp'] = df['timestamp'].dt.round('4h')
    elif frequency == 'hour' or frequency == '1hour' or frequency == '60min':
        # Round to nearest hour
        df['timestamp'] = df['timestamp'].dt.round('1h')
    elif frequency in ['30min', '15min', '5min']:
        # Special handling for sub-hour frequencies that return sparse daily-like data
        # Check if we're getting pseudo-daily data (only 1 timestamp per day)
        timestamps_per_day = df.groupby(df['timestamp'].dt.date).size()
        if timestamps_per_day.max() <= 2:  # Pseudo-daily data detected
            logger.warning(f"Detected pseudo-daily data for {frequency} frequency (only {timestamps_per_day.max()} timestamps per day)")
            # Normalize to midnight to align different hour offsets (04:00 vs 05:00)
            df['timestamp'] = df['timestamp'].dt.normalize()
        else:
            # True intraday data - round to appropriate frequency
            if frequency == '30min':
    ...
    elif frequency == 'minute' or frequency == '1min':
        # Round to nearest minute (usually already aligned)
        df['timestamp'] = df['timestamp'].dt.round('1min')
    # For daily, weekly, monthly - no change needed
    
    # Remove any duplicate timestamps that might result from rounding
    df = df.drop_duplicates(subset=['timestamp'], keep='first')
    
    logger.info(f"Aligned {len(df)} timestamps for frequency {frequency}")
    return df


def merge_ticker_data(df_ticker, df_vix, df_vvix, df_dividends, ticker_name):
    """Merge ticker, VIX, VVIX data and dividends using inner join with timestamp alignment"""
    try:
        ...
        # Apply timestamp alignment for intraday frequencies
        if frequency and frequency not in ['day', 'week', 'month']:
            logger.info(f"Applying timestamp alignment for {frequency} frequency")
            if not df_ticker.empty:
                df_ticker = align_timestamps_for_merge(df_ticker, frequency)
            if not df_vix.empty:
                df_vix = align_timestamps_for_merge(df_vix, frequency)
            if not df_vvix.empty:
                df_vvix = align_timestamps_for_merge(df_vvix, frequency)
        
        # For daily and longer frequencies, just normalize to date
        elif frequency in ['day', 'week', 'month']:
            if not df_ticker.empty:
                df_ticker['timestamp'] = pd.to_datetime(df_ticker['timestamp']).dt.normalize()
            if not df_vix.empty:
                df_vix['timestamp'] = pd.to_datetime(df_vix['timestamp']).dt.normalize()
            if not df_vvix.empty:
                df_vvix['timestamp'] = pd.to_datetime(df_vvix['timestamp']).dt.normalize()
        ...
        if not df_dividends.empty:
            # For intraday frequencies, we need to map dividends by date
            if frequency and frequency not in ['day', 'week', 'month']:
                # Create a date-based mapping for dividends
                df_dividends_copy = df_dividends.copy()
                df_dividends_copy['date'] = df_dividends_copy.index.date
                dividend_map = {}
                for date, group in df_dividends_copy.groupby('date'):
                    dividend_map[date] = group[dividend_col].iloc[0]
                
                # Reset index to work with timestamp column
                merged_df = merged_df.reset_index()
                merged_df['date'] = merged_df['timestamp'].dt.date
                
                # Apply dividends only to the FIRST timestamp of each ex-dividend date
                merged_df[dividend_col] = 0.0
                for date, dividend_amount in dividend_map.items():
                    date_mask = merged_df['date'] == date
                    if date_mask.any():
                        # Find the first timestamp of this date
                        date_rows = merged_df[date_mask]
                        first_idx = date_rows.index[0]
                        # Apply dividend only to the first timestamp
                        merged_df.loc[first_idx, dividend_col] = dividend_amount
                        logger.info(f"Applied dividend of {dividend_amount} to {date} at index {first_idx}")
                
                # Drop the temporary date column
                merged_df = merged_df.drop('date', axis=1)
            else:
                # For daily/weekly/monthly, use regular join
                merged_df = merged_df.join(df_dividends, how='left')
                merged_df[dividend_col] = merged_df[dividend_col].fillna(0)
                # Reset index after join
                merged_df = merged_df.reset_index()
        else:
            # Add empty dividend column and reset index
            merged_df = merged_df.reset_index()
            merged_df[dividend_col] = 0
...
def merge_ticker_data_with_daily_vix(df_ticker, df_vix_daily, df_vvix_daily, df_dividends, ticker_name):
    ...
    try:
        ...
        # Join with dividends - Apply only to FIRST timestamp of each day
        dividend_col = f'{ticker_name}_Dividends'
        if not df_dividends.empty:
            # Get dividend amounts per date
            dividend_map = df_dividends[dividend_col].to_dict()
            
            # Reset index to work with columns
            merged_df = merged_df.reset_index()
            
            # Initialize dividend column to 0
            merged_df[dividend_col] = 0.0
            
            # Apply dividends only to the first timestamp of each ex-dividend date
            for date, dividend_amount in dividend_map.items():
                date_mask = merged_df['date_only'] == date
                if date_mask.any():
                    # Find the first timestamp of this date
                    date_rows = merged_df[date_mask]
                    first_idx = date_rows.index[0]
                    # Apply dividend only to the first timestamp
                    merged_df.loc[first_idx, dividend_col] = dividend_amount
                    logger.info(f"Applied dividend of {dividend_amount} to {date} at first timestamp")
            
            # Drop date_only as we'll recreate it if needed
            merged_df = merged_df.drop('date_only', axis=1)
        else:
            # Reset index and add empty dividend column
...
def generate_candlestick_chart(df, ticker, frequency):
    ...
    try:
        ...
        # Data decimation for large datasets (improves performance)
        chart_df = df.copy()
        if len(chart_df) > 1000:
            # For datasets larger than 1000 points, sample every nth row
            sample_rate = max(1, len(chart_df) // 1000)
            chart_df = chart_df.iloc[::sample_rate]
            logger.info(f"Decimated {ticker} data from {len(df)} to {len(chart_df)} points for performance")
        ...
        fig.update_layout(
            ...
            # Optimize hover mode for performance
            hovermode='x unified',
            hoverlabel=dict(
                bgcolor="black",
                font_size=12,
            ...
            xaxis=dict(
                ...
                # Optimize range selector for performance
        ...
        <style>
        /* Fix cursor appearance for Plotly charts */
        .plotly .nsewdrag {{
        ...
        var chartDiv = document.getElementById('{div_id}');
        if (chartDiv && window.Plotly && chartDiv.data && chartDiv.data[0]) {{
            var dataLength = chartDiv.data[0].x ? chartDiv.data[0].x.length : 0;
            if (dataLength > 500) {{
                Plotly.setPlotConfig({{use_webgl: true}});
            }}
        }}
        </script>
        '''
...
def generate_chart_api(request):
    ...
    if request.method == 'POST':
        try:
            ...
            # No fallback to daily data - timestamp alignment should handle all cases
            # If merge fails, it means there's no overlapping data
